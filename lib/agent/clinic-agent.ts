...
async function replyToQuestion(
  ...
  if (scheduleTopic(text)) {
    ...
    if (asksForSlot(text)) {
       ...
       */
      const known = await knownService(ctx.companyId, [text, ...said.map((t) => t.content)]);
      return respond(ctx, conversation.id, {
        text: known
          ? "Свободное время подберёт администратор — он напишет здесь же."
          : "Свободное время подберёт администратор — он напишет здесь же. " +
            "Пока скажите, пожалуйста, на какую услугу записываемся и для кого — " +
            "взрослому или ребёнку. Передам вместе с вашим вопросом, чтобы вам не повторяться.",
      });
    }
