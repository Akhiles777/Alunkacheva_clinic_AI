 ...
 */
const CONSENT_ASK =
  /(?:соглас\p{L}*\s+на\s+(?:\p{L}+\s+){0,2}обработк|обработк\p{L}*\s+персональн|обработку\s+персональных\s+данных|согласны\s+ли\s+вы|ответьте\s+«?да»?\s+или\s+«?нет»?|(?:ответьте|напишите|отправьте)\s*:?\s*[«"„]?\s*соглас|(?:нужн\p{L}*|требуется|необходимо|понадоб\p{L}*)\s+(?:\p{L}+\s+){0,2}соглас|(?:подтвердите|дайте|предоставьте)\s+(?:ваше\s+)?соглас)/iu;
