 ...
 *
 * Убираем такие предложения, когда согласие уже есть. Остальной текст — цена,
 * услуга, вопрос по делу — остаётся.
 */
const CONSENT_ASK =
  /(?:соглас\p{L}*\s+на\s+обработку|обработку\s+персональных\s+данных|согласны\s+ли\s+вы|ответьте\s+«?да»?\s+или\s+«?нет»?)/iu;
