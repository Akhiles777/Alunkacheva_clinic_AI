...
/**
 * «Доброго дня» — такое же приветствие, как «добрый день».
 *
 * Список знал только именительный падеж, и родительный уходил общим путём:
 * человек здоровался, а получал карточку случайной услуги с ценой.
 */
describe("приветствия в родительном падеже", () => {
  it("узнаются как приветствие", () => {
    expect(isGreeting("Доброго дня")).toBe(true);
    expect(isGreeting("Доброго утра!")).toBe(true);
    expect(isGreeting("Доброго времени суток")).toBe(true);
  });

  it("смайлик приветствию не мешает", () => {
    expect(isGreeting("Доброго дня ☘️")).toBe(true);
  });

  it("вопрос рядом с приветствием остаётся вопросом", () => {
