...
export function alreadyGreeted(history: Turn[], greeting?: string): boolean {
  ...
  return history.some((t) => {
    ...
    /**
     * Приветствие бывает не первым словом — перед ним обращение.
     *
     ...
     *
     * Снимаем обращение из одного-двух слов и смотрим начало снова. Искать
     * приветствие где угодно в предложении нельзя: «работаем в будни, добрый
     * день у нас начинается рано» сошло бы за приветствие, и настоящего
     * человек бы не услышал.
     */
    const head = said.split(/[.!?\n]/, 1)[0] ?? "";
    /**
     * Пробуем и как есть, и без обращения впереди. Снимать обращение всегда
     * нельзя: «Здравствуйте, Патимат!» — там первым словом как раз
     * приветствие, и снятие «обращения» унесло бы именно его.
     */
    const withoutAddress = head.replace(/^\p{L}+(?:\s+\p{L}+)?,\s*/u, "");
    // Граница слова через отсутствие соседней буквы: `\b` в JavaScript
    ...
    const greets =
      /^(здравствуйте|добрый день|доброе утро|добрый вечер|приветствую|привет|салам|ассаламу алейкум)(?!\p{L})/u;
    return greets.test(head) || greets.test(withoutAddress);
  });
}
