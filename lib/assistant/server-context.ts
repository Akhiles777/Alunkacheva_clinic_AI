...
export async function buildClinicSnapshot(companyId: string, now = new Date()): Promise<string> {
   ...
   *
   * Продажей считается заявка НОВОГО пациента, которую ассистент довёл сам:
   * он попросил данные, человек их прислал, администратору осталось поставить
   * время. Запись, оформленная администратором от начала до конца, и запись
   * постоянного пациента сюда не идут — иначе число приписывает себе весь
   * поток клиники.
   */
  ...
  if (salesMonth) {
    ...
    lines.push(
      `Новых пациентов довёл до записи: ${salesMonth.bookings}; из них состоялось: ${salesMonth.arrived}; ` +
        `деньги состоявшихся: ${money(salesMonth.revenue)}.`,
    );
    lines.push(
      "Считается только запись НОВОГО пациента, где данные попросил сам ассистент и получил " +
        "ответ. Назначенный приём — план, а не выручка: деньги считаем у состоявшихся визитов.",
    );
