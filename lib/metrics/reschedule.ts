...
/**
 * Ключ настройки: с какого момента переносы действительно записываются.
 *
 * Не дата первого переноса и не дата миграции. Первая прячет метрику, пока
 * переносов нет, — а «ноль переносов» после двух дней работы тоже ответ.
 * Вторая врёт: таблица появилась 11 сентября, но до 13-го выгрузка переносы не
 * замечала вовсе (они писались только в пути вебхука, которого у клиники нет),
 * и «счёт ведётся с 11-го» выдало бы два пустых дня за «не переносили».
 */
export const MOVES_TRACKING_KEY = "movesTrackingSince";
