...
/**
 * Ключ устройства в учёте: метка браузера, если она есть, иначе отпечаток.
 *
 * Отпечаток строки браузера не различает одинаковые аппараты: два iPhone на
 * одной версии iOS дают дословно одну строку, и в `/sistem` владелец видел их
 * одним устройством — «моё» скрывало оба. Метка (`mera_device`) — случайный
 * номер, который браузер получает при входе и хранит у себя; у каждого
 * аппарата он свой. Действия, записанные до метки, по-прежнему узнаются по
 * отпечатку: переписать прошлое журнала нечем.
 *
 * Префикс «b:» не пересекается с отпечатком (16 шестнадцатеричных знаков), и
 * ключ помещается в прежнее поле `KnownDevice.fingerprint` без изменения схемы.
 */
export function deviceKey(uaFingerprint: string, deviceId: string | null | undefined): string {
  return deviceId ? `b:${deviceId}` : uaFingerprint;
}

/**
 * Короткий номер помеченного устройства для экрана: «№3f9a».
 *
 * Подпись «iPhone · Safari» у двух телефонов одинаковая, и без номера строки
 * не отличить глазами. У устройств без метки номера нет — честно.
 */
export function deviceTag(key: string): string | null {
  return key.startsWith("b:") ? `№${key.slice(2, 6)}` : null;
}
