...
export interface AnswerFacts {
  ...
  /**
   * Известно ли, идёт курс или человек выпал из графика.
   *
   * «Выпал» — вердикт ЭКРАНА: он считается по порогу услуги и запасному порогу
   * клиники («Кому позвонить», «Курсы»). Там, где этих порогов под рукой нет,
   * повторять правило нельзя — это была бы вторая правда о том же курсе (§8).
   * Тогда называем только счёт сеансов и отправляем за вердиктом в «Курсы».
   */
  courseStateKnown?: boolean;
  /**
...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    answer: (_q, f) => {
      ...
      const known = f.courseStateKnown !== false;
      return (
        f.courses
          .map(
            (c) =>
              ...
              (known ? ` — ${state[c.status]}` : ""),
          )
          .join("; ") + (known ? "." : ". Выпал ли человек из графика — в разделе «Курсы»: там порог клиники.")
      );
    },
  },
  {
