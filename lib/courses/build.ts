...
export interface CourseVisit {
  ...
  revenue: number;
  /**
   * Визит уже состоялся (ARRIVED).
   *
   * У запланированного приёма в записи YCLIENTS стоит цена из прайса — это
   * план, а не принятые деньги: она обнуляется, когда администратор закрывает
   * сеанс на курс. Пока этого различия не было, будущие сеансы курса
   * выглядели оплаченными отдельно: у пациентки, купившей три курса
   * (28 000 + 28 000 + 26 000), десять предстоящих сеансов по 2 800 ₽ не
   * попали ни в один курс, третья покупка так и не открылась, а экран
   * подписал их «сеанс 21 при купленных 20 — курс кончился».
   */
  happened: boolean;
}
...
export function buildCourses(visits: CourseVisit[], opts: BuildCoursesOptions): CoursePlan {
  ...
  for (const v of [...visits].sort((a, b) => a.startAt.getTime() - b.startAt.getTime())) {
    /**
     * Деньгами считаем только то, что уже приняли.
     *
     * Цена у ещё не состоявшегося приёма — план из прайса, и при закрытии
     * сеанса на курс она обнулится. Считать её оплатой значит выкидывать
     * будущие сеансы из курса и не открывать уже купленный курс.
     */
    if (v.revenue > 0 && v.happened) {
      ...
      /**
       * Сумма не похожа на цену курса — значит это обычный сеанс, которому
       * администратор проставил цену из прайса.
       *
       * Раньше такой визит выбрасывался из разбора совсем, и получалось
       * круговое рассуждение: сеанс не в курсе, потому что за него взяли
       * деньги, а деньги за него оставили, потому что он не в курсе. У
       * пациентки, купившей 30 сеансов и сходившей ровно 30 раз, два приёма
       * стояли по 2 800 ₽ — те же рубли, что она заплатила при покупке курса.
       *
       * Поэтому решает не цена, а наличие оплаченного места: есть открытый
       * курс со свободным сеансом — визит его сеанс, и денег дня он не даёт
       * (их дал день покупки). Нет — значит за приём действительно заплатили
       * отдельно, и сумма остаётся.
       */
    }
    ...
    /**
     * Свободного места не нашлось.
     *
     * Нулевой сеанс при этом остаётся необъяснённым — он из курса, купленного
     * до окна выгрузки; такие считаем отдельно, чтобы их было видно числом.
     * А платный приём объяснён полностью: за него заплатили в этот день, и
     * никакого курса за ним нет.
     */
    if (v.revenue <= 0) orphans.push(v.id);
  }
