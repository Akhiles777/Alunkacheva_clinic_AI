...
describe("сборка курса из записей", () => {
  ...
  /**
   * Приём с ценой внутри оплаченного курса — сеанс этого курса.
   *
   * Прежде такой визит выбрасывался из разбора, и рассуждение выходило
   * круговым: сеанс не в курсе, потому что за него взяли деньги, а деньги
   * оставили, потому что он не в курсе. На живых данных это и был двойной
   * счёт: пациентка купила 30 сеансов, сходила ровно 30 раз, и два приёма
   * стояли по 2 800 ₽ — те же рубли, что она отдала при покупке курса.
   */
  it("платный приём внутри курса — его сеанс, а не отдельная оплата", () => {
    const plan = buildCourses(
...
describe("предстоящие сеансы и цена из прайса", () => {
  ...
  it("состоявшийся приём с ценой расходует курс, пока в нём есть место", () => {
    const plan = buildCourses([visit("1", 1), visit("2", 2, 2800)], {
      ...BOS,
      sales: [{ id: "s1", at: day(1), amount: 28000 }],
    });
    expect(plan.courses).toHaveLength(1);
    expect(plan.courses[0].visitIds).toEqual(["1", "2"]);
  });

  /**
   * Свободного места нет — значит за приём действительно заплатили.
   *
   * Это граница правила: курс из десяти сеансов израсходован, одиннадцатый
   * визит с ценой ни к какому курсу не относится, и его деньги — выручка дня.
   */
  it("после израсходованного курса платный приём остаётся платным", () => {
    const visits = [
    ...
    expect(plan.courses).toHaveLength(1);
    expect(plan.courses[0].visitIds).toHaveLength(10);
    expect(plan.courses[0].visitIds).not.toContain("extra");
    // И «сиротой» он тоже не считается: он объяснён — за него заплатили.
    expect(plan.orphans).not.toContain("extra");
  });
