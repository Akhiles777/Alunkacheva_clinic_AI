    ...
    const [appts, existingCourses] = await Promise.all([
      prisma.appointment.findMany({
        where: {
           ...
           * не платили ни сегодня, ни при продаже.
           *
           * У КУРСОВОЙ услуги такой записи больше не бывает: скидка 100% на
           * ней означает «оплачено курсом» и приезжает как PREPAID
           * (`courseAwareSource`). Раньше эти сеансы отбрасывались здесь, и
           * курс из десяти показывал «7/10, выпал из графика» у пациентки,
           * сходившей восемь раз. Условие оставлено как страховка на старых
           * данных, ещё не перечитанных выгрузкой.
           */
