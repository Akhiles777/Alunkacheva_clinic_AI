...
const svc = (id: string, title: string, price: number, visits: number): ServiceRow => ({
  id,
  title,
  price,
  visits,
...
describe("цена курса из справочника услуг", () => {
  it("«БОС-терапия, курс» задаёт цену курса для «БОС-терапия»", () => {
    ...
    const out = coursePriceByService([
      ...
      svc("bos-course", "БОС-терапия, курс", 28000, 0),
    ]);
    expect(out.get("bos")).toBe(28000);
  });
  ...
  it("из двух курсовых карточек берётся полная", () => {
    const out = coursePriceByService([
      svc("bos", "БОС-терапия", 2800, 694),
      svc("c4", "БОС-терапия, курс 4", 11000, 0),
      svc("c10", "БОС-терапия, курс 10", 28000, 0),
    ]);
    expect(out.get("bos")).toBe(28000);
  });
