...
/**
 * Скидка 100% на курсовой услуге — это «оплачено курсом», а не подарок.
 *
 * В YCLIENTS сеанс закрывают на абонемент двумя способами. Правильный —
 * списанием: стоимость становится нулём сама, без скидки. Но администратор
 * часто делает проще: ставит скидку 100%, и в записи остаётся `discount: 100`,
 * `cost: 0`.
 *
 * Различить их по записи нельзя, а последствия у нас были разные. Скидку 100%
 * мы считали подарком, и сеанс не занимал места в курсе: сборка курсов такие
 * визиты отбрасывала. Из-за этого у пациентки курс из десяти сеансов показывал
 * «7/10, выпал из графика», хотя она сходила восемь раз, а сам сеанс стоял в
 * карточке подписью «бесплатно» — под названием «БОС-терапия», которая стоит
 * 2 800 ₽. Владелец справедливо спросил, как такое возможно.
 *
 * Курсовую услугу даром не отдают — её продают курсом. Поэтому ноль на
 * курсовой услуге означает одно: за сеанс уже заплатили. Денег это не меняет
 * (в обоих случаях ноль), меняет только то, засчитывается ли сеанс в курс.
 *
 * На некурсовой услуге скидка 100% остаётся скидкой 100%: детский приём в
 * подарок и контрольный визит, включённый в стоимость основного, — это разные
 * вещи, но обе не про курс.
 */
export function courseAwareSource(
  source: ServiceRevenueSource,
  isCourseService: boolean,
): ServiceRevenueSource {
  return source === "FREE" && isCourseService ? "PREPAID" : source;
}
