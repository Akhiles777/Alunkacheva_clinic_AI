...
function explain(message: string | undefined, status: number): string {
  ...
  if (/идентификатор партнера/i.test(raw)) {
    /**
     * Замеченный случай: токен набирали глазами, и ноль превратился в букву O.
     * Ответ при этом ровно тот же, что у несуществующего партнёра, и найти
     * опечатку без подсказки почти невозможно — в токене нет ни одного слова.
     */
    return (
      "YCLIENTS не узнаёт партнёрский токен. Чаще всего это опечатка при " +
      "наборе: ноль и буква O, единица и I. Скопируйте токен из кабинета " +
      "разработчика кнопкой копирования, а не выделением. Если токен точно " +
      "верный — проверьте, активировано ли приложение партнёра."
    );
  }
