...
export interface Visit {
   ...
   * Состав визита: у каждой услуги своя стоимость.
   *
   * У приёма бывает несколько услуг — мать записывает себя и ребёнка одной
   * записью. Название мы показывали только у первой, а сумму — всю: «Взрослый
   * приём — остеопатия, 13 000 ₽» при цене приёма 8 000. Владелец справедливо
   * спросил, как остеопатия может стоить тринадцать тысяч. Ответ — 8 000 плюс
   * детский приём 5 000, и он обязан быть виден на экране.
   */
  parts?: { title: string; amount: number }[];
