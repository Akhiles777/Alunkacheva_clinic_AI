 ...
 * пару дней и месяц вперёд, без справочников и кассы. `?night=1` — фоновые
 * работы (контроль качества ответов агента и сводка владельцу) принудительно.
 * Полное расписание живёт внутри приложения, но круги доступны и снаружи:
 * когда нужно увидеть запись сейчас, а не через три минуты.
 */
async function dispatch(req: Request): Promise<Response> {
   ...
   */
  /**
   * `?night=1` — фоновые работы сейчас: проверка ответов агента и сводка
   * владельцу. Обе идут по своим окнам, и дождаться их естественным путём
   * можно только не спав или дотерпев до понедельника.
   */
  if (q.get("night")) return NextResponse.json({ ok: true, фоновыеРаботы: await runNightCycle(true) });
  return run();
}
