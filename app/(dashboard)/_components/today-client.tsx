...
export function TodayClient() {
  ...
  /**
   * Приём считается прошедшим, когда он ЗАКОНЧИЛСЯ.
   *
   * Одно правило на два числа — «не отмечено» и «впереди», — потому что это
   * ровно две половины одного множества. Раньше они считались порознь, и
   * запланированный приём, время которого прошло без отметки, попадал в оба
   * сразу: заказчик видел «не отмечено 2» и тут же «впереди 2» про тех же
   * самых людей. Число, посчитанное дважды, хуже отсутствующего — по нему
   * планируют день.
   *
   * По концу, а не по началу: пока приём идёт, отметки и не должно быть, и
   * «впереди» он тоже уже не стоит — он сейчас.
   */
  const over = (a: (typeof planned)[number]) =>
    !isToday || a.startMinute + a.durationMin <= stripMinute;
   ...
   */
  const unmarked = planned.filter(over);
  /** Впереди — только то, что ещё не закончилось. Для прошедшего дня — ничего. */
  const ahead = isToday ? planned.filter((a) => !over(a)) : [];
