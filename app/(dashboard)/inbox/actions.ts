 ...
 *
 * Это граница против бесконечного роста, а не ограничение показа. Список
 * обновляется раз в шесть секунд, и каждая строка тянет за собой последние сто
 * сообщений: без границы запрос растёт вместе с базой, и через несколько лет
 * работы клиники это тысячи переписок и сотни тысяч сообщений на КАЖДЫЙ круг
 * опроса.
 *
 * Число намеренно с большим запасом: по этому же списку ищет ⌘K, и на экране
 * написано, что закрытые переписки находятся поиском. Обрезать его до размера
 * видимого списка значит сделать это обещание неправдой — старую переписку
 * перестали бы находить вовсе.
 */
...
/** По клинике, а не на весь процесс: у каждой свои диалоги. */
const relinkedAt = new Map<string, number>();
...
export async function getConversations(): Promise<DialogRecord[]> {
   ...
   */
  if (Date.now() - (relinkedAt.get(session.companyId) ?? 0) > RELINK_EVERY_MS) {
    relinkedAt.set(session.companyId, Date.now());
