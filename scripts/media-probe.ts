 ...
 * у них ссылка и на каком она хосте.
 *
 * Хост когда-то решал всё: `/api/media` пускал только к `green-api.com`, а
 * файлы лежат в хранилище провайдера (`media-…​.storage.yandexcloud.kz`) — и
 * наша же проверка резала ссылку. Списка больше нет, адрес берётся из своей
 * базы, но видеть хосты полезно: по ним понятно, откуда провайдер отдаёт файлы.
 *
...
async function main() {
  ...
  console.log(
    "\nЧто это значит:\n" +
      ...
      "  «со ссылкой» — файл доступен: /api/media берёт адрес из этой же записи\n" +
      "  и отдаёт его вошедшему сотруднику, каким бы ни был хост хранилища.",
  );
