...
async function main() {
  ...
  if (dups.length === 0) {
    ...
    for (const d of dups.slice(0, 10)) {
      console.log(
        `      ${d.startAt.toISOString().slice(0, 16)} · записи YCLIENTS: ${d.recordIds.map((r) => r ?? "нет").join(", ")}`,
      );
    }
    const bothRemote = dups.filter((d) => d.recordIds.every((r) => r !== null)).length;
    console.log(`  из них обе половины есть в YCLIENTS: ${bothRemote}`);
    console.log(
      "  такие задвоены в самом YCLIENTS — чинить надо там, иначе выгрузка вернёт их обратно (§2).",
    );
    const localHalf = dups.length - bothRemote;
    if (localHalf > 0) {
      console.log(`  остальные ${localHalf}: одна половина заведена у нас и не доехала в YCLIENTS.`);
    }
    console.log("");
  }
