...
async function main() {
  ...
  console.log(
    "  RECORD — стоимость стоит в записи YCLIENTS: деньги приняты в этот день.\n" +
      "  PREPAID — сеанс курса: деньги приняты в день продажи, здесь ноль законен.\n" +
      "  FREE — услуга отдана бесплатно, и это факт, а не пробел.\n" +
      "  UNKNOWN — цену забыли проставить; правится в YCLIENTS.\n" +
      "  PRICE_LIST — старое правило, цена бралась из прайса. После полного\n" +
      "               перечёта таких строк остаться не должно.",
  );
