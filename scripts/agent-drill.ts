...
const DIALOGS_WORKDAYS: Scenario[] = [
  ...
  },
  {
    ...
    expect: "отвечает про КЛИНИКУ, а не списком остеопатов: в субботу есть и другие услуги",
    channel: "WHATSAPP",
    turns: ["Здравствуйте! А в среду вы работаете?", "Хочу записаться"],
  },
  {
    title: "БОС в субботу — дни этого специалиста клиника не задавала",
    expect:
      "не выдумывает: про клинику отвечает, про дни БОС-специалиста молчит и передаёт администратору",
    channel: "WHATSAPP",
    turns: ["Здравствуйте! А БОС-терапия в субботу есть?"],
  },
  {
    title: "«В субботу работаете?» — вопрос про клинику",
    expect: "часы клиники и услуги, а не «в субботу принимает Разият»: в этот день работает вся клиника",
    channel: "WHATSAPP",
    turns: ["Здравствуйте! Вы в субботу работаете?", "А что у вас в субботу можно сделать?"],
  },
];

/**
 * Повседневные разговоры после всех последних правок.
 *
 * Живой диалог, с которого они начались: «ребёнку 6 лет, 2 дня как установили
 * аппарат марка Росса, хотела записаться на консультацию» — и в ответ два
 * лишних вопроса подряд: «запись для ребёнка, верно?» (она это сказала) и «к
 * какому специалисту?» (БОС ведёт один человек).
 */
const DIALOGS_DAILY: Scenario[] = [
  {
    title: "Запись ребёнка на консультацию по БОС",
    expect:
      "не переспрашивает про ребёнка, не спрашивает «к кому» — специалист один, и не больше одного вопроса",
    channel: "WHATSAPP",
    turns: [
      "Добрый вечер, ребенку 6 лет, 2 дня как установили аппарат марка Росса, хотела к вам записаться на консультацию",
      "Да",
      "Баширова Асият Башировна, 6 лет, 20 кг, хочу провести диагностику ребенку",
    ],
  },
  {
    title: "Обычная запись взрослого — старое не сломалось",
    expect: "услуга, цена, согласие, данные — и время оставляет администратору",
    channel: "WHATSAPP",
    turns: ["Здравствуйте! Хочу записаться к остеопату", "Взрослому", "Да"],
  },
  {
    title: "Перенос записи у знакомой пациентки",
    expect: "перенос — работа администратора, агент не пересказывает подробности",
    channel: "WHATSAPP",
    knownPhone: "+79280000001",
    turns: ["Здравствуйте, можно перенести мою запись?", "В 9 утра не смогу"],
  },
];

/**
 * Тот самый диалог, который заказчик прислал последним.
 *
 * Пациентка сказала, что хочет к Ирине, ответила «взрослого», получила цену,
 * ответила «Да» — и в ответ пришёл весь блок справочника про двух остеопатов.
 * Всё уже было выяснено, оставалось записать данные.
 */
const DIALOGS_BOOKING: Scenario[] = [
  {
    title: "Запись к названному врачу: после «Да» просим данные, а не справку",
    expect:
      "не переспрашивает про ребёнка («записаться» — про себя) и после согласия просит ФИО, возраст и вес",
    channel: "WHATSAPP",
    turns: [
      "Здравствуйте",
      "Хочу записаться на прием к остеопату Ирина Алункачевой",
      "Да",
      "Магомедова Нурият Ахмедовна, 41 год, 68 кг, боли в шее",
    ],
  },
];

/**
 * Порядок разговора о записи: цена → согласие → данные.
 *
 * Два живых диалога заказчика, и в обоих нарушен один и тот же порядок.
 * «Хотела девочку записать к остеопату» — и сразу «к какому врачу хотите?»
 * вместе с юридическим согласием: цен человек не видел, а согласие спросили за
 * два шага до данных. «Хочу записать ребенка» — то же самое на вопросе об
 * услуге. После «Да» разговор перескакивал к ФИО, минуя выбор врача.
 */
const DIALOGS_ORDER: Scenario[] = [
  {
    title: "«Хотела девочку записать к остеопату» — детские цены до выбора врача",
    expect:
      "называет ДЕТСКИЕ цены обоих остеопатов, согласие на этом шаге не просит; " +
      "просит данные только после выбора врача",
    channel: "WHATSAPP",
    turns: ["Добрый вечер 🌸", "Хотела девочку записать к остеопату", "К Разият Ризвановне"],
  },
  {
    title: "«Хочу записать ребенка» — согласие не на вопросе об услуге",
    expect:
      "уточняет услугу без согласия, на «к остеопату» называет детские цены, " +
      "согласие спрашивает один раз и ровно перед данными",
    channel: "WHATSAPP",
    turns: ["Здравствуйте", "Хочу записать ребенка", "К остеопатц", "К Ирине Алилгаджиевне"],
  },
];
