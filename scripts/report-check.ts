...
async function main() {
  ...
  /**
   * Привязка к курсу и нулевая сумма — не одно и то же.
   *
   * Прежняя строка читала их как одно («159 из 158») и на ровном месте
   * выглядела ошибкой. Сеанс курса, к которому в том же приёме добавили
   * платную услугу, привязан к курсу и при этом приносит деньги — это верно.
   * Настоящая ошибка обратная: нулевой курсовой визит без привязки. Значит
   * курс не собрался, а сеанс объяснить нечем.
   */
  const courseSessions = arrived.filter((a) => a.courseId !== null).length;
  const prepaid = srcAcc.get("PREPAID")?.n ?? 0;
  const prepaidUnlinked = arrived.filter(
    (a) => a.revenueSource === "PREPAID" && a.courseId === null,
  ).length;
  console.log(`  сеансов, привязанных к курсу: ${courseSessions}; из них без своей суммы ${prepaid - prepaidUnlinked}`);
  console.log(
    prepaidUnlinked === 0
      ? "  ✓ курсовых визитов без курса нет: каждому нулю есть объяснение"
      : `  ✗ ${prepaidUnlinked} курсовых визитов без привязки к курсу — ноль, который нечем объяснить.` +
          "\n      Курс не собрался: продажи нет в кассе либо услуга не отмечена курсовой.",
  );
