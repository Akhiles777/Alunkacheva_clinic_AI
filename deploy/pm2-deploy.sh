...
# Типы ЖИВОЙ сборки убираем до проверки новой.
#
# tsconfig.json включает `.next/types/**` — сгенерированные описания страниц
# той сборки, что сейчас работает. Новая сборка идёт в `$BUILD_DIR`, но проверка
# типов читает все включённые файлы, в том числе старые. Стоит удалить страницу
# — и старое описание ссылается на файл, которого больше нет: «Cannot find
# module '../../app/(dashboard)/chat/page.js'», сборка падает, выкатка не
# происходит. Со стороны это выглядит хуже, чем есть: сервер работает, но на
# ПРЕДЫДУЩЕЙ версии, и все правки после неё молча не доезжают.
#
# Работающему приложению эти файлы не нужны: `next start` их не читает, это
# описания для проверки типов, а не код.
rm -rf "$LIVE_DIR/types" "$LIVE_DIR/dev/types"
